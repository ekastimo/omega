...
export const crmConstants = {
    ...
    crmAddCompany: "crmAddCompany",
    crmEditCompany: "crmEditCompany",
...
export default function reducer(state = initialState, action: any): ICrmState {
    switch (action.type) {
        ...
        case crmConstants.crmEditCompany: {
        ...
        case crmConstants.crmDeleteBankAccount: {
            const id: string = action.payload
            console.log("Deleting",{id,sel:state.selected})
            if (state.selected) {
                const {bankAccounts, ...rest} = state.selected
